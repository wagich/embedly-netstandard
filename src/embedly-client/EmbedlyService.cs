...
namespace Embedly
{
	public class EmbedlyService : IEmbedlyService
	{
		...
		public async Task<IEmbedContent> LoadContent(string url, string additionalUrlParameters = null)
		{
			var requestUrl = $"https://api.embed.ly/{_apiVersion}/oembed?key={_apiKey}&format=json&maxwidth={_maxWidth}&maxheight={_maxHeight}&url={Uri.EscapeDataString(url)}";

			if (!string.IsNullOrEmpty(additionalUrlParameters))
			{
				requestUrl += $"&{additionalUrlParameters}";
			}

			var payload = await (await _client.GetAsync(requestUrl)).Content.ReadAsStringAsync();
		...
		public async Task<EmbedExtract> ExtractContent(string url)
		{
			...
			var payload = await (await _client.GetAsync($"https://api.embed.ly/{_apiVersion}/extract?key={_apiKey}&format=json&maxwidth={_maxWidth}&maxheight={_maxHeight}&url={Uri.EscapeDataString(url)}")).Content.ReadAsStringAsync();
		...
		public async Task<List<EmbedExtract>> ExtractContent(string[] urls)
		{
			...
			var payload = await (await _client.GetAsync($"https://api.embed.ly/{_apiVersion}/extract?key={_apiKey}&format=json&maxwidth={_maxWidth}&maxheight={_maxHeight}&urls={urlList}")).Content.ReadAsStringAsync();
